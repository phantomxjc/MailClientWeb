...
- **新邮件提醒**（可选）：收件箱来新邮件时，通过**你自己申请**的推送服务（Server 酱 / PushPlus / Bark / 企业微信·钉钉·飞书群机器人 / 任意 Webhook）发到手机。不走浏览器通知——内网 `http://` 访问拿不到通知权限；推送用的 Key 与邮箱授权码一样加密存库（详见「新邮件提醒」）
...
- **逐封 / 合并**。默认一封一条。一轮同步来了 5 封就发 5 条——Server 酱免费版每天只有
  5 条额度，想省额度就把「合并推送」勾上，多封合成一条。
